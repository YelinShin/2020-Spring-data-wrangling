...
```{r warning=FALSE,echo=FALSE}
# find top 10 world rank by GDP
GDP.ranking <- "https://en.wikipedia.org/wiki/List_of_countries_by_GDP_(nominal)"  %>% read_html() %>% html_table(fill=TRUE) %>% .[[6]]

GDP.ranking30 <-  GDP.ranking %>% 
...
GDP.ranking30$region <- tolower(GDP.ranking30$region)

matched2 = inner_join(wtime_covid_final["region"], GDP.ranking30["region"], by ="region")
rename3 = setdiff(GDP.ranking30$region,wtime_covid_final$region)
rename3

GDP.ranking30$region[GDP.ranking30$region == "united states"] <- "united states of america"
GDP.ranking30$region[GDP.ranking30$region == "korea, south"] <- "south korea"
```
...
```{r, warning=FALSE,echo=FALSE}
#confirmed line graph by top30 countries ~ time
a= wtime_covid_final %>%
  group_by(region)%>%
  filter(region %in% GDP.ranking30$region) %>%
  plot_ly(x = ~Date, y = ~confirmed, color = ~region, type = 'scatter', mode = 'lines')

#confirmed/population line graph by top30 countries ~ time
b= wtime_covid_final %>%
  filter(!is.na(population))%>%
  mutate(confirmed.rate = (confirmed/population)*100) %>%
  group_by(region)%>%
  filter(region %in% GDP.ranking30$region) %>%
  plot_ly(x = ~Date, y = ~confirmed.rate, color = ~region, type = 'scatter', mode = 'lines')
...
a1 <- list(
  text = "Confirmed rate change \n in top 30 countries (by GDP)",
  font = f,
  xref = "paper",
  yref = "paper",
  yanchor = "bottom",
...
subplot(a %>%layout(annotations = a1),b %>% layout(annotations = b1),margin = 0.07)
```

```{r,echo=FALSE}
w.test_ratio = w_latest_covid %>%
...
test.bar = ggplot(data = w.test_ratio, aes(x = region, y=test.ratio))+ theme(axis.text.x = element_text(angle=45, hjust = 1)) +  geom_bar(stat="identity")

ggplotly(test.bar) %>% layout(title = "Cumulative test number / population rate in the world")


top30.test_ratio=  w.test_ratio%>%
  filter(region %in% GDP.ranking30$region)

top30.test_ratio$region <- factor(top30.test_ratio$region, levels = top30.test_ratio$region[order(match(top30.test_ratio$region,GDP.ranking30$region))]) 


test.bar.30 = ggplot(data = top30.test_ratio, aes(x = region, y=test.ratio))+ theme(axis.text.x = element_text(angle=90, hjust = 1)) +  geom_bar(stat="identity")

#since some country doesn't share the cumulative test number, it won't contain all 30 countries.
ggplotly(test.bar.30) %>% layout(title = "Cumulative test number / population rate in top country")
