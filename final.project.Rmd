...
# Data set (1):
  ...
  The main world time series dataset is from github, "https://github.com/datasets/covid-19/blob/master/data/time-series-19-covid-combined.csv". Since the time series corona case file contains 2018 population of each countries, I decided to change the number to 2019 population from wikipedia, https://en.wikipedia.org/wiki/List_of_countries_by_population_(United_Nations) by web-scrapping the page. Also, I grab the time-series update of the number of tested case in each countries by "https://github.com/owid/covid-19-data/blob/master/public/data/testing/covid-testing-all-observations.csv". This csv file contains too many information and the country name contains '- tests performed', I extract the country name only and the current update of testing number. Moreover, some countries have 2 resources to track the testing number. So I fix one resource per country. After I got the 3 cleaned data, I use join to get the finalized data that contains all the cases, test, and population. Moreover, I make a table for latest updated case number for each countries. Since some countries did not update/share the testing number everyday, I grab most recent number of testing into this table. Lastly, I get the world ranking by GDP from wikipedia ("https://en.wikipedia.org/wiki/List_of_countries_by_GDP_(nominal)" ). Since there is too many countries in the data, it is better to show subset of countries. Therefore, it is good to show the economically developed countires for data visuialization.

  For all data source, I edit the countries' name to match with 'country.regions' to use chroplethrMaps. However, some of the countries are not covered by 'country.regions' so it will not appear in maps.
...
## Most updated number of cases & time-series for cases
  ...
  The number of actives in world is still over 2 million cases. And I am quite surprise that the number of recovered is almost one-thrid of confirmed cases. It indicates that good amount of confirmed people get cured by medicine or self-recovered. Since lots of countries' hospitals face frontlines of crisis because of coronavirus, the number of people get recovered is important number to see for checking whether the virus blows over.
  
  The time-line of each case is helpful to understand overall situation and changes.
\  
...
## World map by active cases number and rate

  Since we have worldwide active case, it is good to visualize what continent/country have more active case than other. 

  I tried to plot the graph in two ways because the actives case is depend on the population of country. When you see the bottom two tables, the top active countries would not show up in top active rate countries since their number is relatively small in population.
...
```
\  
  When we see the bottom two graphs, some countries have more lighter or darker color in active rate map compare to active number itself. Norway, Ireland, Gabon, and Chile have darker color in rate map.
...
## Confirmed cases and Confirmed ratio by population
  ...
  To look the number deeply, I plot the number of confirmed cases and confirmed rate by time line in specific countries. I plot the rate by population because it makes easy to compare various countries in a one plot. Also, I chose top 10 GDP ranked countries' cases since most of people have interested to see the number in developed countries, and how they deal with this situation. 
  
  Before March, most countires have very low confirmed cases, except China. So I checked up the first date of testing case in countries. Even though some countries started testing in January and Feburary, they did not get confirmed case that much. However, after mid-March the graph starts to increase a lot especially in United States. 

  However, rapid increase in U.S. confirmed cases does not mean that U.S. people tend to have positive result in testing than other countries. Therefore, if I look the right plot (confirmed rate), actually Italy has higher confirmed rate than U.S. Also most of countries have more rapid change in rate. 
...
## Testing number changes 
  ...
  After I figure out the confirmed case is depend on population, I realize I have to compare cumulative testing number change also. Since lots of countries do not have enough number of covid-19 testing kit, there might be some countries have lower confirmed cases because they did not test enough. Also, some people said the number of testing in developed countries bursted their bubble. They thought developed countries can deal with the virus, but in fact, they also facing difficulty with supplying sanitized product such as mask, sanitizer, gloves, and etc.  
...
```
Note: Since China and Brazil did not provide their number of testing, the testing numer change did not contain those two countries.

## Testing number changes 

  By looking at the upper plot, it shows that top rank countries does not gurantee they test covid-19. Japan is rank #2 country, but they are the least testing country in May. 
...
## Distribution of active, recover, and death in confirmed case

  To see the distribution well, I grab the top 15 countries who have most confirmed case. The proportion is inconsistent between countries. However, the interesting part is death proportion. Except Russia,Peru, Germany, it is quite noticeable. 
...
## Distribution of death cases by races in United States
  ...
  Since the world death rate (deaths / confirmed) is around 7%, I tried to look into more detail information like the distribution of race. Since U.S. is most diverse country and the confirmed number is siginificantly high, I get U.S. death information by race from Centers for Disease Control and  Prevention (CDC) , "https://www.cdc.gov/nchs/nvss/vsrr/covid_weekly/index.htm", since they test most among other countries. 
...
\  
  ...
  After looking at the distribution of death in U.S., there were significantly large percetnatge of death report from White than other races. By looking at this, obviously we cannot conclude a specific race tend to dead more/less from coronavirus. But good resource to see the trend in diverse country.

## Conclusion:

  I faced a difficulty to grab right data for testing number. Since some of the countries does not share their testing number, it was hard to show the relationship between testing and confirm or other cases. Even some of them count the number from 2 different sources and they does not match. Also, some of the countries does not update the testing from their source frequently. 
  
  However, after visualize the results, I found out higher ranked countries does not gurantee they can handle the situation better than other countries. Some of lower rank countries did more testing and got more recovered cases. We can do further research  about relapse cases if we have the data. 
  
  Also, by the active graph, even though there were some countries have down trend after April, we should keep eyes on the trend becasuse there is a possibility of having second wave of coronavirus. 